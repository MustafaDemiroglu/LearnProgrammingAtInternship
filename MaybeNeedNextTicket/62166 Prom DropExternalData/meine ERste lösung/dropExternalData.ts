...
export class DropExternalDataWindow extends Window {
	...
	private addTooltipEventHandler(): void {
		this.linkField.getEl().on('mouseenter', () => {
			if (this.linkField.val() === "" && this.group != null && this.group.getValue() === "link") {
				this.linkField.getEl().attr("title", t("prom:filePathTooltip"));
			} else {
				this.linkField.getEl().removeAttr("title");
			}
		});
	}
