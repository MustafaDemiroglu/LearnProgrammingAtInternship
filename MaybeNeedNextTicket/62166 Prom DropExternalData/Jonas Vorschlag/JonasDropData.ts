...
export class DropExternalDataWindow extends Window {
	...
	private linkField: Textfield | null = null;
	private confirmBtn!: Button;
	private $btnGroup: JQuery | null = null;
	private $warnMessage: JQuery;

	constructor(description: string, url: string, xPosition: number, yPosition: number, callback: (check: any, description?: any, url?: any, release?: any, createLink?: any) => void) {
		super();
		...
		if (url) {
			this.urlField = this.createInputField(t("prom:url"), url, undefined, t("prom:urlTooltip"));
		} else {
		...
		const $warnMessage = this.createLabel(t("prom:filePathTooltip"), "warnMessage");
		this.$warnMessage = $warnMessage;

		this.createBtnGroup(callback);
		this.show();

		this.linkField?.getEl().attr("title", t("prom:filePathTooltip"));
	...
	private createInputField(name: string, val: string, placeholder?: string, tooltip?: string): Textfield {
		...
		textField.title(tooltip ? tooltip : "");
	...
	private checkValidConfirmBtn(): void {
		...
		this.descriptionField.bindLiveEvents(() => {
			this.checkFieldValid();
		});
		if (this.urlField != null) {
			this.urlField.bindLiveEvents(() => {
				this.checkFieldValid();
			});
		}
	}
	*/
	...
	private checkValidConfirmBtn(): void {
		...
		if (this.linkField != null) {
			this.linkField.bindLiveEvents(() => {
				this.checkFieldValid();
			});
		}
		if (this.group != null) {
			this.group.getEl().find('input[type="radio"]').on('change', () => {
	...
	private checkFieldValid(): void {
		if (this.descriptionField.val() === "" || (this.urlField != null && this.urlField.val() === "")) {
	...
	*/
	...
	private checkFieldValid(): void {
		...
		const canConfirm = isDescriptionValid && isUrlValid &&
			(!isLinkChecked || (isLinkChecked && isLinkValid));
