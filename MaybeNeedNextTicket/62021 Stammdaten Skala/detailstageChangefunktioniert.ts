...
import { SkalaModel } from './model/SkalaModel';
		...
		if (tableOptionModel != null) {
			...
			closeBtn.bindClick(() => {
				...
				this.pageList.forEach((page) => {
					...
					if (additionalModel && !$.isEmptyObject(additionalModel.getChangedValues())) {
						if (additionalModel instanceof SkalaModel) {
							skalaModified += additionalModel.getModified() ? 1 : 0;
						} else {
							modelsModified = true;
						}
					}

					modelsModified = modelsModified || skalaModified > 0;
